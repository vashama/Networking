...
function deriving_lost_time($st)
{
    ...
    for($i = 1;$i < $a;$i++)
    {
        $v = strcmp($ar[$i-1], "received,");
        $v1 = strcmp($ar[$i+1], "packet");  
       if(($v == 0) && ($v1 == 0) )
                  $l = $ar[$i];
}
    return $l;
...
function deriving_ping_time($str)
{
    $arr = explode(" ", $str);
    $k = count($arr);
    $l = array();
...
function deriving_round_trip_time($st)
{
    ...
    for($i = 0;$i < $a;$i++)
       if(strpos($ar[$i], "round-trip") !== false)
        if(strpos($ar[$i+1], "min/avg/max/stddev") !== false)
...
$ipaddr = $_POST['ipaddr'];
$cmd = 'ping -c 10 ' . $ipaddr;
$output = shell_exec($cmd);
$lost = "Packet Loss: " . deriving_lost_time($output);
$t = deriving_ping_time($output);
...
?>
    ...
    <head>
      ...
      function drawChart() {
        ...
        ["icmp_seq=0", <?php echo $t[1];?>, "#003B5A"],
        ["icmp_seq=1", <?php echo $t[2];?>, "#006CA5"],
        ["icmp_seq=2", <?php echo $t[3];?>, "#9BDDFF"],
        ["icmp_seq=3", <?php echo $t[4];?>, "#50C3FF"],
        ["icmp_seq=4", <?php echo $t[5];?>, "#00F2FF"],
        ["icmp_seq=5", <?php echo $t[6];?>, "#3D94B8"],
        ["icmp_seq=6", <?php echo $t[7];?>, "#50C3FF"],
        ["icmp_seq=7", <?php echo $t[8];?>, "#006CA5"],
        ["icmp_seq=8", <?php echo $t[9];?>, "#00FFFF"],
        ["icmp_seq=9", <?php echo $t[10];?>, "#003B5A"]
        ]);
        ...
        data1.addRows([
          ...
          ['STANDERD DEVIATION', <?php echo $t1[3];?>],
        ]);
      ...
      var options = {
        ...
        var options1 = {'title':'Ping ROUND-TRIP',
                       'width':1000,
                       'height':600,
                       'position': 'center'};
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    <ul class="nav navbar-nav navbar-right">
                        <li>
                            <a href="ping.html"><h4><strong>Ping Tool</strong></h4></a>
                        </li>
                        <li>
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-sm-10 col-sm-offset-1 text">
                            <h1><strong>PING TESTING</strong></h1>
                            <h5>
                            <?php echo $lost;?>
                         </h5>
                            <h5>
